...
describe Kiitos::KiitosMailer do
  describe '.received_kiito_notification' do
    before do
      ...
      Kiitos::KiitosMailer.received_kiito_notification('to@example.com', @message).deliver
    end
    ...
    it 'sends message details' do
      ...
      assert message.body.raw_source.include?(@message.message), 'Did not include the message.'
      assert message.body.raw_source.include?('Go to your dashboard.'), 'Did not include link to dashboard'
    end
  end
end
